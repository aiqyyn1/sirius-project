...
const resetSend = async (req, res) => {
      try {
            ...
            await mailService.sendResetMail(email, `https://sirius-sdu.vercel.app/auth/reset/${token}`)
...
const resetChange = async (req, res) => {
      try {
            ...
            if(resetLink){
                  jwt.verify(resetLink, secretReset.secret, async function (err, decodedData) {
                        if(err){
                              console.log(err);
                        ...
                        if(!user){
                              return res.status(401).json({message: 'User with this reset token does not exist'})
                        }
...
module.exports = { Registration, Login, getUser, refresh, logout, resetSend, resetChange};
